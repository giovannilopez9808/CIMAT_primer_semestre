...
+ dimension: arreglo de dimension 2 de tipo entero
 */
void read_diagonal_dimension(FILE *file, int *dimension)
{
    int aux;
    int a = fscanf(file,
                   "%d %d %d",
...
+ matrix: doble puntero de un tipo de dato double donde se alojaran los datos de la matriz
 */
void read_diagonal_matrix(FILE *file, int *dimension, double **matrix)
{
    *matrix = (double *)malloc((dimension[0]) * (dimension[1]) * sizeof(double));
    int a, i, j;
    double value;
    ...
    {
        a = fscanf(file, "%d %d %lf", &i, &j, &value);
        *(*matrix + (j - 1) * dimension[0] + (i - 1)) = value;
    }
    (void)a;
}
/*
