    ...
    for (int i = 0; i < dimension_matrix[0]; i++)
    {
        for (int j = i; j < dimension_matrix[0]; j++)
        {
            ...
            if (i != j)
            {
                ...
                {
                    printf("Matriz invalida para el método i=%d j=%d\n", i, j);
                    printf("ij=%lf ji=%lf", matrix_ij, matrix_ji);
                    exit(1);
                }
            }
        }
...
void obtain_Cholesky(double *matrix, int dimension_matrix[], double **L, double **LT)
{
     ...
     */
    double *LT_ii, *L_ij;
    double sqrt_number, sum, lt_ik, lt_jk, matrix_ii, matrix_ij;
    ...
    for (int i = 0; i < dimension_matrix[0]; i++)
    {
        ...
        *LT_ii = sqrt(sqrt_number);
        ...
        for (int j = i; j < dimension_matrix[1]; j++)
        {
            ...
            L_ij = (*LT + j * dimension_matrix[0] + i);
            // Obtiene el elemento ij de la matrix
            matrix_ij = *(matrix + i * dimension_matrix[0] + j);
            for (int k = 0; k < i; k++)
            {
                lt_ik = *(*LT + i * dimension_matrix[0] + k);
                lt_jk = *(*LT + j * dimension_matrix[0] + k);
                sum += lt_ik * lt_jk;
            }
            // Obtiene el elemento L_ij
            *L_ij = (matrix_ij - sum) / *LT_ii;
        }
    }
    // Completa la matriz LT a partir de L
    fill_L_transpose(*LT, *L, dimension_matrix);
}