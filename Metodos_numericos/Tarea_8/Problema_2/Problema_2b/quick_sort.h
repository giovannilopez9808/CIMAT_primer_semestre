...
int reduce_sort(int *numbers, double *positions, int low, int high)
{
    ...
    // Realiza el cambio de valores
    swap_int(numbers + i, numbers + high);
    // Realiza el cambio de posiciones
    swap(positions + i, positions + high);
    return i;
}

void sorted(int *numbers, double *positions, int start, int last)
{
    /* 
    ...
    if (start < last)
    {
        ...
        numbers_partition = reduce_sort(numbers,
                                        positions,
                                        start,
                                        last);
